...
void conv(float *x, float *conv_weight, float *in_proj_weight, float *out_proj_weight,
    ...
    // -----------------------------------------------------------------------
    {
        ...
        matmul_kernel<BM, BN, BK, TM, TN><<<gridDim, blockDim>>>(x_gpu, in_proj_weight_gpu, in_proj_out_gpu, M, N, K, M_sub);
  ...
  // -----------------------------------------------------------------------
    ...
    // -----------------------------------------------------------------------
    {
        ...
        matmul_kernel<BM, BN, BK, TM, TN><<<gridDim, blockDim>>>(y_pre_transposed_gpu, out_proj_weight_gpu, output_gpu, M, N, K, M_sub);
        CHECK_CUDA(cudaGetLastError());
    }
